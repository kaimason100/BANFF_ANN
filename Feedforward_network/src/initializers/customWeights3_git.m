function weights = customWeights3_git(sz)
%CUSTOMWEIGHTS3_GIT   Generate reproducible balanced ternary weights (10×16000 pool).
%   WEIGHTS = CUSTOMWEIGHTS3_GIT(SZ) returns a matrix of size SZ
...
%
%   Inputs:
%     sz   — 1×2 vector specifying [number of rows (≤10), number of columns (≤16000)].
%
%   Outputs:
%     weights — sz(1)×sz(2) submatrix of the scaled ternary pool.
%
%   Example:
%     W = customWeights3_git([5, 1024]);
%
%   Notes:
%     • Full random pool has dimensions 10×16000; sz must not exceed these.
...
%     • RNG seed fixed to 1 for deterministic output.
    ...
    maxRows = 10;
    maxCols = 16e3;
    assert(sz(1) <= maxRows && sz(2) <= maxCols, ...
        'sz dimensions must not exceed [%d, %d].', maxRows, maxCols);

    % Fix the random seed for reproducibility
    rng(5678910 + getNarWeightSeedBase());
    ...
    % Preallocate the full weight pool
    A = zeros(rows, cols);
    ...
    % Scale by 1/sqrt(16000) to normalize
    W3 = A / sqrt(maxCols);

    % Return the requested submatrix
    weights = W3(1:sz(1), 1:sz(2));
end
