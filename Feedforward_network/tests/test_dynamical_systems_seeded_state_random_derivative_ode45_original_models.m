% AUTO-GENERATED from the normal DS test. Run this file in MATLAB.
% Numerical evaluation is unchanged; model and output locations are redirected.

% Locate setup relative to the script, with a current-folder fallback.
startFolders = {fileparts(mfilename('fullpath')), pwd};
packageFolder = '';
for startIndex = 1:numel(startFolders)
    candidate = startFolders{startIndex};
    while ~isempty(candidate)
        if isfile(fullfile(candidate,'setup.m')), packageFolder = candidate; break; end
        if isfile(fullfile(candidate,'Feedforward_network','setup.m'))
            packageFolder = fullfile(candidate,'Feedforward_network'); break
        end
        parent = fileparts(candidate);
        if strcmp(parent,candidate), break; end
        candidate = parent;
    end
    if ~isempty(packageFolder), break; end
end
assert(~isempty(packageFolder),'Run setup.m from the Feedforward_network folder first.');
run(fullfile(packageFolder,'setup.m'));
...
figureDir = fullfile(repoRoot, 'outputs', 'test_figures', 'dynamical_systems_seeded_state_random_derivative_ode45_original_models');
...
if SAVE_RESULTS
    ...
    outFile = fullfile(outDir, 'dynamical_systems_seeded_state_random_derivative_ode45_original_models_test_results.mat');
...
modelPath = fullfile(repoRoot, 'trained_networks_old', char(networkSet), folderName, ...
...
if isstring(continuationNetworkSets) && isscalar(continuationNetworkSets) && strcmpi(continuationNetworkSets, "auto")
    listing = dir(fullfile(repoRoot, 'trained_networks_old', 'seeded_state_random_derivative_ode45_continued_lr_0p001_extra_*'));
