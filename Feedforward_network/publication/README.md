...
- Dynamical-system publication trajectories use `[0 5000]` for every system,
  including Lorenz.
- The default publication IC uses seed `9100` and an independent uniform
  standardized perturbation in `[-0.01, 0.01]` for each state coordinate,
  matching the active DS test.
