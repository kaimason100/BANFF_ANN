...
The feedforward experiments use the datasets listed below, but this repository
does not distribute dataset copies. Users obtain them from the cited upstream
sources and prepare local MAT files according to `Feedforward_network/docs/DATASETS.md`.
The upstream licences apply to those local copies; the repository MIT licence
does not supersede them.

### UCI datasets: CC BY 4.0

The following upstream datasets use the Creative Commons Attribution 4.0
...
The expected local files are `abalone_dataset.mat`,
`breast_cancer_dataset.mat`, `car_dataset.mat`, and
`mushroom_dataset.mat`.

### Toyota used-car data: CC0 1.0

Prepare `toyota_dataset.mat` from the Toyota subset of Aditya Desai's
...
Prepare `mnist.mat` from the MNIST database by Yann LeCun, Corinna Cortes and
...
Prepare `mnist_fashion.mat` from Fashion-MNIST, copyright 2017 Zalando SE:
...
Prepare `kmnist.mat` from Kuzushiji-MNIST by ROIS-DS CODH:
...
Prepare `afro_mnist_ethiopic.mat`, `afro_mnist_nko.mat`,
`afro_mnist_osmanya.mat`, and `afro_mnist_vai.mat` from Afro-MNIST by Daniel
J. Wu, Andrew C. Yang and Vinay U. Prabhu:
